...
def graficar(Punto_1, Punto_2, Punto_3, mitad_compliance, p=1.3, lado="", guardar_como=None):
    x0, y0 = Punto_1
    ...
    if lado == "izquierdo":
        ...
        color_curva = "steelblue"

    plt.figure(figsize=(8, 5))
    plt.plot(x_plot, y_plot, color= color_curva, linewidth=2, label="Curva de compliancia")
    plt.scatter([Punto_1[0], Punto_2[0], Punto_3[0]],
                [Punto_1[1], Punto_2[1], Punto_3[1]],
                color="black", marker="x", s=80, linewidths=2, zorder=5,
    ...
    plt.title(f"Timpanograma {lado}".strip())
    plt.grid(alpha=0.3)
    plt.legend()
    plt.xlim(-300, 300)
    plt.ylim(0, 1.5)
